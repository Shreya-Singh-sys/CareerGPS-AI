...
//   const role = searchParams.get("role") || "General Candidate";
...
// //       setQuestions(data);
// //     } else {
// //       // BACKEND FAIL HONE PAR STATIC QUESTIONS USE KAREIN
// //       console.warn("Using fallback static questions");
// //       setQuestions(getQuestions(roleToUse)); 
// //     }
// //   } catch (error) {
// //     console.error("Backend offline, using static questions");
// //     setQuestions(getQuestions(roleToUse));
// //   } finally {
// //     setIsGenerating(false);
// //     setStarted(true); // Isse finally mein rakhein taaki UI hamesha show ho
// //     setCurrentQ(0);
// //     setSubmitted(false);
// //   }
// // };
...
//   useEffect(() => {
//     if (initialRole) {
//       setRoleSearch(initialRole);
//       setSelectedRole(initialRole);
//       // Direct handleStart() call karne ki jagah ek function banayein
//       fetchAIQuestions(initialRole);
//     }
//   }, [initialRole]);
...
//         {isGenerating ? (
//         <motion.div key="loading" className="text-center py-20 space-y-4">
//           <RotateCcw className="h-10 w-10 animate-spin mx-auto text-primary" />
//           <p className="text-muted-foreground animate-pulse">AI is crafting custom questions for {selectedRole}...</p>
//         </motion.div>
//         ) : !started ? (
...
//       {/* 3. Yahan par title show karein */}
//       <h1 className="text-2xl font-bold mb-4">
//         Mock Interview for: <span className="text-primary">{role}</span>
//       </h1>
      
//       <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
//         <p className="text-muted-foreground mb-6">
//           AI is preparing questions specifically for the **{role}** position. 
...
const MockInterview = () => {
  ...
  const toggleRecording = () => {
    ...
    if (!isRecording) {
      setTimeout(() => {
        setAnswer((prev) => prev + (prev ? " " : "") + "Based on my experience, I approach this with clear planning and execution...");
        setIsRecording(false);
      }, 2000);
    }
  };
            ...
            <Card className="border-0 shadow-lg bg-gradient-card">
              ...
              <CardContent className="space-y-4">
                <Input
                  value={roleSearch}
                  onChange={(e) => { setRoleSearch(e.target.value); setSelectedRole(""); }}
                  placeholder={t("Type any role (e.g. Software Engineer)...")}
                  className="h-11"
                />
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto">
                  {filteredRoles.map((role) => (
                ...
                >
                  {t("Start Interview")} <ArrowRight className="h-4 w-4 ml-2" />
                </Button>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div key="finished" {...fadeUp(0.1)} className="space-y-6">
            <Card className="border-0 shadow-lg bg-gradient-card">
              <CardContent className="p-8 text-center space-y-4">
                ...
                <h2 className="text-2xl font-display font-bold text-foreground">{t("Interview Complete!")}</h2>
                <p className="text-muted-foreground">{t("You completed the interview for")} {selectedRole}</p>
                <Button onClick={handleRestart} variant="outline" className="gap-2">
                  <RotateCcw className="h-4 w-4" /> {t("Try Another Role")}
                </Button>
              </CardContent>
            ...
            <Card className="border-0 shadow-lg bg-gradient-card">
              <CardContent className="p-6 space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-lg font-medium text-foreground">{questions[currentQ]?.question}</p>
                </div>
                ...
                {!submitted ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="bg-gradient-primary text-primary-foreground h-11"
                      >
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 pt-2">
                    ...
                    <Button onClick={handleNext} className="bg-gradient-primary text-primary-foreground">
                      {currentQ < questions.length - 1 ? t("Next Question") : t("Finish Interview")}
                      <ChevronRight className="h-4 w-4 ml-1" />
                    </Button>
                  </motion.div>
